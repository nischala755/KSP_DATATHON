...
type IngestionResult = { status: string; filename: string; language: string; extracted: { fir_number: string | null; station: string | null; district: string | null; date: string | null; sections: string[]; complainant: string | null; narrative: string; raw_text: string }; ocr: { provider: string; model: string; pages: number; average_confidence: number | null }; persisted: boolean; scope_note: string }
...
function IngestionView() {
  ...
  return <section className="workspace-page"><div className="page-intro"><div><span className="eyebrow">Human-confirmed extraction</span><h2>Scanned FIR ingestion</h2><p>Upload a scanned FIR for OCR-assisted field extraction. Nothing is persisted until an authorized operator reviews and confirms it.</p></div><FileSearch /></div>
    ...
    {result && <div className="review-card"><div className="review-head"><div><span className="eyebrow">Operator review required</span><h2>Extraction preview</h2><small>{result.ocr.provider} · {result.ocr.model} · {result.ocr.pages} page{result.ocr.pages === 1 ? '' : 's'}{result.ocr.average_confidence != null ? ` · ${Math.round(result.ocr.average_confidence * 100)}% OCR confidence` : ''}</small></div><span>{result.persisted ? 'Persisted' : 'Not persisted'}</span></div><dl><div><dt>File</dt><dd>{result.filename}</dd></div><div><dt>Detected language</dt><dd>{result.language}</dd></div><div><dt>FIR number</dt><dd>{result.extracted.fir_number || 'Not detected'}</dd></div><div><dt>Police station</dt><dd>{result.extracted.station || 'Not detected'}</dd></div><div><dt>District</dt><dd>{result.extracted.district || 'Not detected'}</dd></div><div><dt>Date</dt><dd>{result.extracted.date || 'Not detected'}</dd></div><div><dt>Sections</dt><dd>{result.extracted.sections?.join(', ') || 'Not detected'}</dd></div><div><dt>Complainant</dt><dd>{result.extracted.complainant || 'Not detected'}</dd></div><div className="wide"><dt>Extracted narrative</dt><dd>{result.extracted.narrative}</dd></div><div className="wide raw-ocr"><dt>Raw OCR text</dt><dd>{result.extracted.raw_text}</dd></div></dl><p><ShieldCheck /> {result.scope_note}</p></div>}
  </section>
}
